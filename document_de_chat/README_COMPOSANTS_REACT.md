...
function MessageBubble({ msg, isOwn }) {
  return (
    <div style={{ background: isOwn ? "blue" : "grey" }}>{msg.content}</div>
  );
}
```
...
const sendMessage = useCallback(
  ...
  [emit],
); // ne recrée la fonction que si emit change
```
...
export default function ChatPage() {
  ...
  const { rooms, currentRoom, messages, fetchRooms, joinRoom, sendMessage } =
    useChat();
  const { user } = useAuth();
...
export default function MessageBubble({ msg, isOwn, onReply }) {
  ...
  const content = () => {
    ...
    if (msg.type === "audio")
      return <audio controls src={msg.attachment.secureUrl} />;
    if (msg.type === "giphy") return <img src={msg.attachment.url} />;
    if (msg.type === "file")
      return (
        <a href={msg.attachment.secureUrl}>📎 {msg.attachment.filename}</a>
      );
