    ...
    <div className="p-2 sm:p-4 relative">
      ...
      <div className="flex items-end gap-1.5 sm:gap-3 bg-slate-800/50 rounded-xl sm:rounded-2xl border border-slate-700/50 px-2 py-2 sm:px-4 sm:py-3">
        ...
        <textarea
          ...
          placeholder={currentRoom ? `Message dans ${currentRoom.name}` : 'Rejoins un salon...'}
          value={text}
...
function RoomItem({ room, isActive, onClick, unreadCount, isCreator, onEdit, onDelete }) {
  ...
  return (
    ...
    >
      ...
      <div onClick={onClick} className="flex items-center gap-1.5 flex-1 min-w-0">
        {isActive ? (
          <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
          </svg>
        ) : (
          <svg className={`w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 ${hasUnread ? 'text-white' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
          </svg>
        )}
        <span className={`text-xs sm:text-sm truncate ${hasUnread ? 'font-bold text-white' : 'text-gray-300'}`}>{room.name}</span>
