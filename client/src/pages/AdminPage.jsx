...
const Ic = {
  ...
  rooms:    () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>,
  ...
  disable:  () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18.36 6.64A9 9 0 0 1 20.77 15"/><path d="M6.16 6.16a9 9 0 1 0 12.68 12.68"/><line x1="2" y1="2" x2="22" y2="22"/></svg>,
  ...
  members:  () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>,
};
...
const STATUS_LABELS = { online:'En ligne', busy:'Occupé', invisible:'Invisible', offline:'Hors ligne' };
...
function Msg({ msg }) {
  ...
  return (
    <div style={{ padding:'10px 14px', borderRadius:8, fontSize:'0.85rem', marginTop:10,
...
function Modal({ title, onClose, children, width = 500 }) {
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:400, padding:16 }}
      onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={{ background:'#16162a', borderRadius:16, width:'100%', maxWidth:width, maxHeight:'90vh', overflowY:'auto',
        boxShadow:'0 24px 64px rgba(0,0,0,0.6)', border:'1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 22px 0' }}>
          <h3 style={{ color:'#f1f5f9', fontWeight:700, fontSize:'1.05rem', margin:0 }}>{title}</h3>
          <button onClick={onClose} style={{ background:'rgba(255,255,255,0.08)', border:'none', color:'#9ca3af', width:30, height:30, borderRadius:7, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <Ic.close />
          </button>
        </div>
...
const S = {
  ...
  panel:   { background:'#111827', width:'100%', maxWidth:1100, borderRadius:'20px 20px 0 0', display:'flex', flexDirection:'column', height:'88vh', overflow:'hidden', boxShadow:'0 -8px 40px rgba(0,0,0,0.7)', border:'1px solid rgba(255,255,255,0.07)' },
  ...
  card:    { background:'#1a1f2e', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:'16px 20px' },
  table:   { width:'100%', borderCollapse:'collapse' },
  th:      { color:'#6b7280', fontSize:'0.75rem', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', padding:'8px 10px', textAlign:'left', borderBottom:'1px solid rgba(255,255,255,0.06)' },
  td:      { padding:'10px 10px', borderBottom:'1px solid rgba(255,255,255,0.04)', color:'#d1d5db', fontSize:'0.87rem', verticalAlign:'middle' },
  badge:   (c='#6366f1') => ({ display:'inline-flex', alignItems:'center', gap:4, padding:'3px 9px', borderRadius:20, fontSize:'0.73rem', fontWeight:600, background:c+'22', color:c, border:`1px solid ${c}44` }),
  avatar:  (color) => ({ width:32, height:32, borderRadius:'50%', background: color||'#6366f1', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:'0.85rem', flexShrink:0 }),
  statCard:(c='#6366f1') => ({ background:'#1a1f2e', border:`1px solid ${c}33`, borderRadius:12, padding:'16px 20px', flex:1 }),
  iconBox: (c='#6366f1') => ({ width:40, height:40, borderRadius:10, background:c+'22', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:8 }),
};
...
function TabStats({ token }) {
  ...
  if (loading) return <p style={{ color:'#6b7280', textAlign:'center', padding:40 }}>Chargement...</p>;
  if (!stats)  return <p style={{ color:'#ef4444' }}>Erreur de chargement</p>;

  const cards = [
    { label:'Utilisateurs',    value:stats.totalUsers,    color:'#6366f1', icon:<Ic.users /> },
    { label:'Connectés',       value:stats.onlineUsers,   color:'#10b981', icon:<Ic.stats /> },
    { label:'Messages total',  value:stats.totalMessages, color:'#f59e0b', icon:<Ic.rooms /> },
    { label:'Messages / jour', value:stats.messagesToday, color:'#ec4899', icon:<Ic.msg /> },
    { label:'Salons',          value:stats.totalRooms,    color:'#8b5cf6', icon:<Ic.rooms /> },
    { label:'Admins',          value:stats.adminCount,    color:'#14b8a6', icon:<Ic.users /> },
    { label:'Bannis',          value:stats.bannedUsers,   color:'#ef4444', icon:<Ic.ban /> },
    { label:'Désactivés',      value:stats.disabledUsers, color:'#f97316', icon:<Ic.disable /> },
  ];
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(140px, 1fr))', gap:10 }}>
        {cards.map(({ label, value, color, icon }) => (
          <div key={label} style={S.statCard(color)}>
            ...
            <p style={{ color:'#9ca3af', fontSize:'0.78rem', margin:'0 0 4px' }}>{label}</p>
...
function ModalEditUser({ user, token, onClose, onSaved }) {
  const [form, setForm] = useState({ fullName: user.fullName||'', email: user.email||'', phone: user.phone||'', bio: user.bio||'', role: user.role||'user' });
  ...
  return (
    <Modal title={`Modifier — ${user.username}`} onClose={onClose}>
      {[['Nom complet','fullName','text'],['Email','email','email'],['Téléphone','phone','tel'],['Bio','bio','text']].map(([lbl,key,type]) => (
        <div key={key} style={{ marginBottom:12 }}>
          <label style={S.label}>{lbl}</label>
          <input type={type} value={form[key]} onChange={set(key)} style={S.input} />
        </div>
      ))}
      ...
      <button onClick={save} disabled={saving} style={S.btn()}>{saving ? 'Sauvegarde...' : 'Enregistrer'}</button>
      <Msg msg={msg} />
    </Modal>
  );
}
...
function ModalResetPwd({ user, token, onClose }) {
  ...
  return (
    <Modal title={`Réinitialiser — ${user.username}`} onClose={onClose} width={400}>
      <label style={S.label}>Nouveau mot de passe</label>
      <input type="password" value={pwd} onChange={(e) => setPwd(e.target.value)} placeholder="Min. 6 caractères" style={{ ...S.input, marginBottom:14 }} />
      <button onClick={reset} disabled={saving || pwd.length < 6} style={S.btn('#ef4444')}>
        <Ic.key />{saving ? 'Réinitialisation...' : 'Réinitialiser'}
      </button>
      <Msg msg={msg} />
    </Modal>
  );
...
function ModalCreateUser({ token, onClose, onCreated }) {
  const [form, setForm] = useState({ username:'', password:'', fullName:'', email:'', phone:'', role:'user' });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState(null);
  const set = (k) => (e) => setForm(p => ({ ...p, [k]: e.target.value }));
  ...
  const create = async () => {
    ...
    try {
      ...
      onCreated(d);
      setForm({ username:'', password:'', fullName:'', email:'', phone:'', role:'user' });
    } catch (e) { setMsg({ type:'error', text: e.message }); }
    setSaving(false);
  };
    ...
    <Modal title="Nouvel utilisateur" onClose={onClose}>
      <div style={S.row}>
        <div style={{ flex:1 }}>
          ...
          <input value={form.username} onChange={set('username')} style={S.input} placeholder="ex: jean_dupont" />
        </div>
        <div style={{ flex:1 }}>
          <label style={S.label}>Mot de passe *</label>
          <input type="password" value={form.password} onChange={set('password')} style={S.input} placeholder="Min. 6 caract." />
        </div>
      </div>
      <div style={S.row}>
        <div style={{ flex:1 }}>
          <label style={S.label}>Nom complet</label>
          <input value={form.fullName} onChange={set('fullName')} style={S.input} />
        </div>
        <div style={{ flex:1 }}>
      ...
      <button onClick={create} disabled={saving || !form.username || form.password.length < 6} style={S.btn()}>
        <Ic.plus />{saving ? 'Création...' : 'Créer'}
      </button>
      <Msg msg={msg} />
    </Modal>
  );
...
function TabUsers({ token }) {
  ...
  const flash = (type, text) => { setActionMsg({ type, text }); setTimeout(() => setActionMsg(null), 3500); };

  const toggleDisable = async (u) => {
    const r = await fetch(`${API}/admin/users/${u._id}/disable`, { method:'PATCH', headers:{ Authorization:`Bearer ${token}` } });
    const d = await r.json();
    if (r.ok) { setUsers(p => p.map(x => x._id === u._id ? { ...x, isDisabled: d.isDisabled } : x)); flash('success', `Compte ${d.isDisabled ? 'désactivé' : 'réactivé'}`); }
    else flash('error', d.error);
  };

  const toggleBan = async (u) => {
    const r = await fetch(`${API}/admin/users/${u._id}/ban`, { method:'PATCH', headers:{ Authorization:`Bearer ${token}` } });
    const d = await r.json();
    if (r.ok) { setUsers(p => p.map(x => x._id === u._id ? { ...x, isBanned: d.isBanned } : x)); flash('success', `Utilisateur ${d.isBanned ? 'banni' : 'débanni'}`); }
    else flash('error', d.error);
  };

  const deleteUser = async (u) => {
    const r = await fetch(`${API}/admin/users/${u._id}`, { method:'DELETE', headers:{ Authorization:`Bearer ${token}` } });
    const d = await r.json();
    if (r.ok) { setUsers(p => p.filter(x => x._id !== u._id)); flash('success', 'Utilisateur supprimé'); }
    else flash('error', d.error);
    setConfirmDel(null);
  };
    ...
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16, flexWrap:'wrap' }}>
        <div style={{ position:'relative', flex:1, minWidth:180 }}>
          <span style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', color:'#6b7280' }}><Ic.search /></span>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher..." style={{ ...S.input, paddingLeft:32 }} />
        </div>
        <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} style={{ ...S.input, width:140 }}>
          <option value="">Tous les rôles</option>
          <option value="user">Utilisateur</option>
          <option value="admin">Admin</option>
        </select>
        <button onClick={load} style={S.btn('#374151', true)}><Ic.refresh /></button>
        <button onClick={() => setShowCreate(true)} style={S.btn()}><Ic.plus /> Ajouter</button>
      </div>
      ...
      {/* Modals */}
      {showCreate && <ModalCreateUser token={token} onClose={() => setShowCreate(false)} onCreated={(u) => { setUsers(p => [u, ...p]); setShowCreate(false); }} />}
      {editUser   && <ModalEditUser  token={token} user={editUser} onClose={() => setEditUser(null)} onSaved={(u) => { setUsers(p => p.map(x => x._id === u._id ? u : x)); setEditUser(null); flash('success', 'Profil mis à jour'); }} />}
      {resetUser  && <ModalResetPwd  token={token} user={resetUser} onClose={() => setResetUser(null)} />}
      {confirmDel && (
        <Modal title="Confirmer la suppression" onClose={() => setConfirmDel(null)} width={380}>
          <p style={{ color:'#d1d5db', marginBottom:16 }}>Supprimer définitivement <strong style={{ color:'#f1f5f9' }}>{confirmDel.username}</strong> ? Cette action est irréversible.</p>
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={() => deleteUser(confirmDel)} style={S.btn('#ef4444')}><Ic.trash /> Supprimer</button>
            <button onClick={() => setConfirmDel(null)} style={S.btn('#374151')}>Annuler</button>
          </div>
        </Modal>
      )}
    </div>
...
function ModalRoomMessages({ room, token, onClose }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const r = await fetch(`${API}/admin/rooms/${room._id}/messages`, { headers:{ Authorization:`Bearer ${token}` } });
  ...
  const deleteMsg = async (m) => {
    const r = await fetch(`${API}/admin/messages/${m._id}`, { method:'DELETE', headers:{ Authorization:`Bearer ${token}` } });
    ...
    <Modal title={`Messages — #${room.name}`} onClose={onClose} width={640}>
      {loading ? <p style={{ color:'#6b7280', textAlign:'center' }}>Chargement...</p> : (
        <div style={{ maxHeight:400, overflowY:'auto', display:'flex', flexDirection:'column', gap:8 }}>
          {messages.length === 0 && <p style={{ color:'#4b5563', textAlign:'center' }}>Aucun message</p>}
          {messages.map(m => (
            <div key={m._id} style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'8px 10px', background:'#1e2538', borderRadius:8 }}>
              <div style={{ ...S.avatar(m.author?.avatar), flexShrink:0, width:28, height:28, fontSize:'0.75rem' }}>
                {m.author?.username?.[0]?.toUpperCase()}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  <span style={{ color:'#a5b4fc', fontSize:'0.8rem', fontWeight:600 }}>{m.author?.username}</span>
                  <span style={{ color:'#4b5563', fontSize:'0.72rem' }}>{formatDate(m.createdAt)} {formatTime(m.createdAt)}</span>
                </div>
                <p style={{ color:'#d1d5db', fontSize:'0.85rem', margin:'2px 0 0', wordBreak:'break-word' }}>
                  {m.type === 'image' ? '[Image]' : m.type === 'audio' ? '[Audio]' : m.type === 'giphy' ? '[GIF]' : m.content}
                </p>
              </div>
              <button onClick={() => setDelMsg(m)} style={{ ...S.btn('#7f1d1d',true), padding:'4px 7px', flexShrink:0 }}><Ic.trash /></button>
            </div>
          ))}
        </div>
      )}
      {delMsg && (
        <div style={{ marginTop:16, padding:'12px 14px', background:'rgba(239,68,68,0.1)', borderRadius:8, border:'1px solid #ef444433' }}>
          <p style={{ color:'#fca5a5', fontSize:'0.88rem', margin:'0 0 10px' }}>Supprimer ce message ?</p>
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={() => deleteMsg(delMsg)} style={S.btn('#ef4444',true)}><Ic.trash /> Supprimer</button>
            <button onClick={() => setDelMsg(null)} style={S.btn('#374151',true)}>Annuler</button>
          </div>
        </div>
      )}
    </Modal>
...
function TabRooms({ token }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const r = await fetch(`${API}/admin/rooms`, { headers:{ Authorization:`Bearer ${token}` } });
  ...
  const deleteRoom = async (room) => {
    const r = await fetch(`${API}/admin/rooms/${room._id}`, { method:'DELETE', headers:{ Authorization:`Bearer ${token}` } });
    const d = await r.json();
    if (r.ok) { setRooms(p => p.filter(x => x._id !== room._id)); flash('success', 'Salon supprimé'); }
    else flash('error', d.error);
    setConfirmDel(null);
  };
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        <h2 style={{ color:'#f1f5f9', fontWeight:700, fontSize:'1.1rem', margin:0 }}>Salons ({rooms.length})</h2>
        <button onClick={load} style={S.btn('#374151', true)}><Ic.refresh /> Rafraîchir</button>
      </div>
      <Msg msg={actionMsg} />
      {loading ? <p style={{ color:'#6b7280', textAlign:'center', padding:30 }}>Chargement...</p> : (
      ...
      {confirmDel && (
        <Modal title="Supprimer le salon" onClose={() => setConfirmDel(null)} width={380}>
          <p style={{ color:'#d1d5db', marginBottom:16 }}>Supprimer <strong style={{ color:'#a5b4fc' }}>#{confirmDel.name}</strong> et tous ses messages ? Action irréversible.</p>
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={() => deleteRoom(confirmDel)} style={S.btn('#ef4444')}><Ic.trash /> Supprimer</button>
            <button onClick={() => setConfirmDel(null)}    style={S.btn('#374151')}>Annuler</button>
          </div>
        </Modal>
      )}
