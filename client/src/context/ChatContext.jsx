...
export const ChatProvider = ({ children }) => {
    ...
    ({ event, data }) => {
      switch (event) {
        ...
        case "new_message": {
          ...
          const isOtherRoom =
            (currentRoomId && messageRoomId !== currentRoomId) ||
            !currentRoomId;
          if (isOtherRoom) {
            ...
            const roomName =
              roomsRef.current.find((r) => String(r._id) === messageRoomId)
                ?.name || "Salon";
  ...
  const sendMessage = useCallback(
    (content, ephemeral = false, ttl = 300, replyTo = null) => {
  ...
  const updateMessage = useCallback(
    async (roomId, messageId, content) => {
      const res = await fetch(`${API}/rooms/${roomId}/messages/${messageId}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
